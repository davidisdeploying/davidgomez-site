...
- **200 concurrent video-frame jobs is a denial-of-service against your own box.** They thrashed a small, memory-limited machine and the network mount into a timeout storm and throughput collapsed. The fix is asymmetric concurrency: throttle the heavy video work (~16 at a time), let the light image work fan wide.
- **The image-writing call picks its codec from the file extension.** Writing to a `.jpg.tmp` temp name fails with "no writer for the specified extension." Encode to a JPEG in memory, then write the bytes atomically under the temp name.
