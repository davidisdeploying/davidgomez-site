...
- Restart the app to actually activate the lazy-render change.
- Public release: endpoint hardening plus a disclosure pass, on my own timeline.
- A now-stale code comment to fix and a logbook compaction that's overdue.
- Cleaned up an old local backup that predated my history scrub, and kept only the clean copy.
